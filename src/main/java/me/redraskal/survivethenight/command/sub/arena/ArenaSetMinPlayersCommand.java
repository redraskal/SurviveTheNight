 ...
 */
public class ArenaSetMinPlayersCommand extends SubCommand {

    @Override
    public String name() {
        return "arena setminplayers";
    }
        ...
        if(args.length > 3) {
            try {
                ...
                if(arenaManager.getArenaMap().containsKey(id)) {
                    ...
                    arena.setMinPlayers(argument);
                    arenaManager.saveArena(id);
                    player.sendMessage(surviveTheNight.buildMessage("<prefix> &aMin players has been set to " + argument + "."));
                } else {
