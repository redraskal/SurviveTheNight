 ...
 */
public class ArenaSetGeneratorsNeededCommand extends SubCommand {

    @Override
    public String name() {
        return "arena setgeneratorsneeded";
    }
        ...
        if(args.length > 3) {
            try {
                ...
                if(arenaManager.getArenaMap().containsKey(id)) {
                    ...
                    arena.setGeneratorsNeeded(argument);
                    arenaManager.saveArena(id);
                    player.sendMessage(surviveTheNight.buildMessage("<prefix> &aGenerators needed has been set to " + argument + "."));
                } else {
